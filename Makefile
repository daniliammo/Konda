...
# `stdlib` — фонимя (существует и каталог stdlib/); иначе make счёл бы цель
# готовой по mtime каталога.
.PHONY: all test stdlib clean install

all: $(TARGET) stdlib/вывод/libkonda_std.so

test: $(TARGET)
	sh tests/run.sh

# Стандартная библиотека — ОДИН монолитный libkonda_std.so (§80), собираемый
# СИСТЕМОЙ СБОРКИ КОНДЫ (`konda build`), а не прямым вызовом транспилятора. Все
# модули (файл — I/O, строки — текст, хеш — FNV, поток — буферизованный вывод)
# остаются РАЗДЕЛЬНЫМИ исходниками (логическое деление для разработки), но
# транспилятор (§18) сливает их в ОДИН артефакт: манифест stdlib/Konda.toml
# («имя = "libkonda_std"» → --имя) эмитит один комплект libkonda_std.{c,h,so,и}
# в stdlib/вывод. Плюсы монолита: один -rpath, меньше стартовых релокаций, инлайн
# между модулями внутри артефакта, одна точка авто-подключения (§35: сканер *.и
# найдёт единственный модуль «libkonda_std»). rm-строка сносит СТАРЫЕ
# пер-модульные артефакты (файл.so/строки.so/хеш.so) — иначе сканер §35 подхватил
# бы их тоже и получилось бы двойное связывание.
STDLIB_ИСХ = stdlib/файл.конда stdlib/строки.конда stdlib/хеш.конда stdlib/поток.конда stdlib/версия.конда stdlib/срезы.конда stdlib/путь.конда stdlib/срезы.конда
STDLIB_ШИМЫ = stdlib/конда_ио.h
STDLIB_МАНИФЕСТ = stdlib/Konda.toml

# Фронтенд «konda build» (сборка/вывод/сборка.elf) — Konda-бинарь, потребляющий
# libkonda_toml. Собирается сборка/собрать.sh (сперва toml.so, затем фронтенд).
ФРОНТЕНД = сборка/вывод/сборка.elf
ФРОНТЕНД_ИСХ = сборка/сборка.конда сборка/сборка_шим.h toml/toml.конда stdlib/хеш.конда stdlib/строки.конда
$(ФРОНТЕНД): $(TARGET) $(ФРОНТЕНД_ИСХ)
	sh сборка/собрать.sh

# Сборка stdlib = запуск фронтенда из каталога stdlib над Konda.toml. Пути к
# транспилятору и фронтенду — абсолютные (KONDA_TRANSPILER + $(CURDIR)), т.к.
# фронтенд запускается со сменой каталога (cd stdlib).
STDLIB_СБОРКА = cd stdlib && rm -f вывод/файл.* вывод/строки.* вывод/хеш.* && \
	KONDA_TRANSPILER="$(CURDIR)/Собранное/ТранспиляторКонда" "$(CURDIR)/$(ФРОНТЕНД)"
stdlib/вывод/libkonda_std.so: $(TARGET) $(ФРОНТЕНД) $(STDLIB_ИСХ) $(STDLIB_ШИМЫ) $(STDLIB_МАНИФЕСТ)
	$(STDLIB_СБОРКА)

# Ручная пересборка stdlib (форсируем через --пересобрать, минуя хеш-кэш
# фронтенда) — для случаев, когда флаги транспилятора менялись.
stdlib: $(TARGET) $(ФРОНТЕНД)
	$(STDLIB_СБОРКА) --пересобрать
