...
# Мини-stdlib (stdlib/файл) — собирается САМИМ транспилятором как библиотека:
# «--библиотека» эмитит «.h» (C-typedef'ы), «.so» (линкуется в потребителей)
# и «.и» (Konda-сигнатуры для проверок через границу). Сгенерированные
# артефакты едут в репозиторий — потребители получают их без пересборки.
stdlib: $(TARGET)
