...
# Стандартная библиотека — ОДИН монолитный libkonda_std.so (§80). Все модули
# (файл — I/O, строки — текст, хеш — FNV) остаются РАЗДЕЛЬНЫМИ исходниками
# (логическое деление для разработки), но сливаются транспилятором (§18) в ОДИН
# артефакт: «--библиотека --имя=libkonda_std <модули…>» эмитит один комплект
# libkonda_std.{c,h,so,и} в stdlib/вывод. Плюсы монолита: один -rpath, меньше
# стартовых релокаций, инлайн между модулями внутри артефакта, одна точка
# авто-подключения (§35: сканер *.и найдёт единственный модуль «libkonda_std»).
# rm-строка сносит СТАРЫЕ пер-модульные артефакты (файл.so/строки.so/хеш.so) —
# иначе сканер §35 подхватил бы их тоже и получилось бы двойное связывание.
STDLIB_ИСХ = stdlib/файл.конда stdlib/строки.конда stdlib/хеш.конда stdlib/поток.конда
STDLIB_ШИМЫ = stdlib/конда_ио.h stdlib/конда_хеш.h stdlib/конда_строки.h
STDLIB_СБОРКА = cd stdlib && ../Собранное/ТранспиляторКонда --библиотека --релиз \
	--имя=libkonda_std файл.конда строки.конда хеш.конда поток.конда
stdlib/вывод/libkonda_std.so: $(TARGET) $(STDLIB_ИСХ) $(STDLIB_ШИМЫ)
	rm -f stdlib/вывод/файл.* stdlib/вывод/строки.* stdlib/вывод/хеш.*
	$(STDLIB_СБОРКА)
