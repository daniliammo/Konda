...
all: $(TARGET) stdlib/вывод/файл.so stdlib/вывод/строки.so stdlib/вывод/хеш.so

test: $(TARGET)
	sh tests/run.sh

# Стандартная библиотека собирается САМИМ транспилятором (это библиотеки
# Konda, а не C): «--библиотека» эмитит .h/.so/.и в stdlib/вывод. Модули
# независимы (файл — I/O, строки — текст, хеш — FNV) → собираются раздельно;
# многомодульное авто-подключение (§35) подхватывает их сканом каталога.
...
stdlib/вывод/строки.so: $(TARGET) stdlib/строки.конда
	cd stdlib && ../Собранное/ТранспиляторКонда --библиотека строки.конда
stdlib/вывод/хеш.so: $(TARGET) stdlib/хеш.конда stdlib/конда_хеш.h
	cd stdlib && ../Собранное/ТранспиляторКонда --библиотека хеш.конда
